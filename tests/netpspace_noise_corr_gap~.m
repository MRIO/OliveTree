...
noisecorr = [0 .1 .2 .4] ; % proportion of shared noise
	...
	for gs = gaps
	   	for ncorr = noisecorr
	   		for namp = noiseamps
				...
				VS = transients{simcount}.networkHistory.V_soma;
				VSB = single(VS);
				VSB(VS<-10)=0;
				VSB(VS>=-10)=1;

				abovethresh = find(sum(VSB'));

				X = xcorr(VSB(abovethresh,1:1000)',150);
			end
		end
	end
...
% [12 16 19 24]
	...
	for simcount = sims2p
		...
		VS = transients{simcount}.networkHistory.V_soma;
		W  = transients{simcount}.networkParameters.connectivityMatrix;
		% NS = transients{simcount}.noiseapplied;

			spks = spikedetect(transients{simcount});
