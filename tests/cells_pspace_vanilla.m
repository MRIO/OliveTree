...
if ~exist('st_st','var')
	...
	st_st = IOnet_new('cell_function', cell_function ,'networksize', netsize, 'cell_parameters', def_neurons, 'time', steadystate_time ,'gpu', gpu,'to_report', to_report ,'delta',delta,'ou_noise', [0 0 0 0],'debug',debugging);
	st_st.Plist = Plist;
...
for gap = gaps
	s = s+1;
   [simresults{s}] = IOnet_new('tempState', st_st.lastState ,'cell_parameters', def_neurons, ...
...
ampl   = table2array(resultstable.allneurons(:,'ampl'))  <20 & table2array(R.allneurons(:,'ampl'))  >2;
freq   = table2array(resultstable.allneurons(:,'freq_each'))  >2 | table2array(R.allneurons(:,'freq_each'))<10 ;
maxV   = table2array(resultstable.allneurons(:,'maxV'))  <-30;
meanVm = table2array(resultstable.allneurons(:,'meanVm'))<-50;
minV   = table2array(resultstable.allneurons(:,'minV'))  <-50;
...
save cellset_vanilla_3 sel_cel_idx simresults resultstable
