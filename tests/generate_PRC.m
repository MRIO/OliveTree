...
% network parameters
	...
	% def_neurons = createDefaultNeurons(noneurons,'celltypes','randomized');
		...
		def_neurons.gbar_ampa_soma = ones(noneurons,1);
...
for pertphase = pertphases
	...
	perturberdPhaseResults = measureGlobalSync(simPert{k},'duration', [1:simtime], 'plotme', 0);
		...
		PP = [PP; perturbedPhases];

	% find new peaks
	[newPKS newLOCS{k}] = findpeaks(mean(perturbedPhases(find(pert.mask{1}) , 100:180)), 'minpeakdistance',40);
