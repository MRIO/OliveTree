...
% pspace tests for networks with afferent ampa input distributed with masks over a period of a certain duration.
...
Plist = Plist(randperm(length(Plist)),:);
...
def_neurons.gbar_gaba_soma = ones(noneurons,1)*.5;
...
W = createW('all to all', [1 noneurons 1], [], 1, 1, 1, [], 15, [], 0, [1 15 1 .001]);
...
ampa_numberofmasks = [10];
ampa_masktype = 'all';
ampa_synapseprobability = .1; % sparse
ampa_intervals = [100 250];
ampa_onset = 100;
ampa_n_of_pulses = 100; % make it a function of the number of pulses == energy delivery
ampa_offset = 2100;

% [=================================================================]
%  Distribute Gaba Perturbation
% [=================================================================]

gaba_numberofmasks = [0];
gaba_masktype = 'all';
gaba_synapseprobability = .5; % dense
gaba_intervals = [10 100];
gaba_onset = 1000;
gaba_offset = 1900; % make it a function of the number of pulses == energy delivery
...
% [===========================================================================================================]
 ...
 for g = gaps
 	...
 	for si = 1:length(ampa_intervals)

	 	ns = ns +1;
	 	thissimtime = simtime;
 		
 		for nm = 1:ampa_numberofmasks
 			n_of_pulses = (ampa_offset - ampa_onset)/ampa_intervals(si);

			pert.mask  	  {nm} = create_input_mask(netsize, ampa_masktype, 'synapseprobability', ampa_synapseprobability);
			pert.amplitude{nm} = 1;
			pert.triggers {nm} = ampa_onset + round(ampa_intervals(si)*rand) + cumsum(poissrnd(ampa_intervals(si),n_of_pulses,1)) ;
			pert.duration {nm} = 1;
			pert.type	  {nm} = 'ampa';
			
		end 

		for nm = ampa_numberofmasks+1:gaba_numberofmasks+ampa_numberofmasks
			n_of_pulses = (gaba_offset - gaba_onset)/gaba_intervals(si);

			pert.mask  	  {nm} = create_input_mask(netsize, gaba_masktype, 'synapseprobability', gaba_synapseprobability);
			pert.amplitude{nm} = 1;
			pert.triggers {nm} = gaba_onset + round(gaba_intervals(si)*rand) + cumsum(poissrnd(gaba_intervals(si),n_of_pulses,1)) ;
			pert.duration {nm} = 1;
			pert.type	  {nm} = 'gaba_soma';
			
		end 

		thissimtime = max(max(cell2mat(pert.triggers'))) + 300;
...
% spks{2} = spikedetect(simresults{2});
