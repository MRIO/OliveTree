function co_plottri(fignum,cpar,thresh)
% DAMOCO Toolbox, function CO_DIRIN, version 05.04.14
%
% Given the matrix cpar of partial norms, the function plots
% the diagram of the coupling structure for three interacting
% oscillators. The matrix entry cpar(n,m) quantifies effect
% of oscillator m on oscillator n. The diagonal elements
% cpar(n,n) characterize the joint action of two oscillators
% on oscillator n, e.g., cpar(2,2) quantifies the action of 
% units 1,3 on 2. Graphically, it is illustrated by an arrow
% from the center of the diagram to oscillator n. For details
% see Ref. [4].
%
% Plotting of lines with arrowhead is performed by the function arrow
% by Dr. Erik A. Johnson, see the code below.
% Copyright (c)1995-2009, Dr. Erik A. Johnson <JohnsonE@usc.edu>, 5/20/2009
% http://www.usc.edu/civil_eng/johnsone/
%
%
% Form of call: co_plottri(fignum,cpar)
%               co_plottri(fignum,cpar,thresh)
% Input:        fignum     : figure window number
%               cpar       : matrix of partial norms
%               thresh     : threshold in percent;
%                            connections that are weaker than
%                            thresh*max(cpar) are not shown
%
figure(fignum);
if nargin==2, thresh=0.07; end

cpar=cpar';    % to match the convention that cpar(1,2) is effect 2-->1
cm=max(max(cpar)); 

x1=0; x2=cos(pi/6); x3=-x2;
y1=1; y2=-sin(pi/6); y3=y2;
x=[x1 x2 x3];  y=[y1 y2 y3];
plot(x,y,'o','MarkerSize',50,'LineWidth',3);
axis([-1.5 1.5 -1.5 1.5]); axis equal;  axis off;
%
for j=1:3
    text(x(j),y(j),num2str(j),'FontSize',24,'HorizontalAlignment','center');
end
%
wm=6;  % Maximal line width
%
cpar=cpar*wm/cm; % rescaling the coupling function
wt=thresh*wm;
%
a31s=[x3+0.25,y3+0.15]; a31e=[x1-0.05,y1-0.26];
if cpar(3,1) > wt
    arrow(a31s,a31e,'Width',cpar(3,1),'Length',30);
end
a13s=[x1-0.25,y1-0.15]; a13e=[x3+0.05,y3+0.25];
if cpar(1,3) > wt
    arrow(a13s,a13e,'Width',cpar(1,3),'Length',30);
end

a12s=[x1+0.05,y1-0.26]; a12e=[x2-0.25,y2+0.15];
if cpar(1,2) > wt
    arrow(a12s,a12e,'Width',cpar(1,2),'Length',30);
end
a21s=[x2-0.03,y2+0.26]; a21e=[x1+0.25,y1-0.13];
if cpar(2,1) > wt
    arrow(a21s,a21e,'Width',cpar(2,1),'Length',30);
end

a23s=[x2-0.25,y2-0.09]; a23e=[x3+0.25,y3-0.09];
if cpar(2,3) > wt
    arrow(a23s,a23e,'Width',cpar(2,3),'Length',30);
end
a32s=[x3+0.25,y3+0.09]; a32e=[x2-0.25,y3+0.09];
if cpar(3,2) > wt
    arrow(a32s,a32e,'Width',cpar(3,2),'Length',30);
end

a01e=[0,y1-0.29];
if cpar(1,1) > wt
    arrow([0 0],a01e,'Width',cpar(1,1),'Length',30);
end
a02e=[x2-0.25,y2+0.11];
if cpar(2,2) > wt
    arrow([0,0],a02e,'Width',cpar(2,2),'Length',30);
end
a03e=[x3+0.25,y2+0.11];
if cpar(3,3) > wt
    arrow([0,0],a03e,'Width',cpar(3,3),'Length',30);
end
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
